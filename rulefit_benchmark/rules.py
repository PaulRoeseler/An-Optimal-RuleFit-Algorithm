...
# Original rule generation utilities pulled from pauls_functions_advanced_v3.py.
# Logic is intentionally preserved; only formatting and module placement changed.
...
def get_feature_type(x, include_binary: bool = False):
...
def get_target_type(x, include_binary: bool = False):
    x.dropna(inplace=True)
    if x.dtype == "float64":
        return "continuous"
    elif x.dtype == "int64":
...
def get_rules(tree_model, feature_names):
...
def gen_rules(df, rules):
    ...
    for rule in rules:
        rule_copy = rule[:-1] + ")"
        rule_eval = rule.replace("feature", "df")

        filter_idx = df.loc[eval(rule_eval)].index.values
...
def generate_tree(
            ...
            if i <= max_iter_hy:
                if sub_paths is not True:
                    model = iai.GridSearch(
                        iai.OptimalTreeRegressor(hyperplane_config={"sparsity": "all"}), max_depth=depth_grid_hy
                    )
                    model.fit(X_train_sample, y_train)
        ...
        if OCT_H is True:
            if i <= max_iter_hy:
                if sub_paths is not True:
                    print("start OCT-H")
                    model = iai.GridSearch(
                        iai.OptimalTreeClassifier(hyperplane_config={"sparsity": "all"}), max_depth=depth_grid_hy
                    )
                    print(1)
                    model.fit(X_train_sample, y_train)
                    print(2)
                    ...
                    for depth in depth_grid_hy:
                        model = iai.OptimalTreeClassifier(
                            max_depth=depth, cp=complexity_hy, hyperplane_config={"sparsity": "all"}
                        )
                        model.fit(X_train_sample, y_train)
                        rules_cla_oct_h.append(get_optimal_rules(model))
                        perf_cla_oct_h.append([model.score(X_test_sample, y_test, criterion="misclassification")])
    ...
    models = [
        ...
        rules_reg_ort_h,
        rules_cla_oct_h,
        rules_reg_ort + rules_reg_ort_h,
        rules_cla_oct + rules_cla_oct_h,
    ]
...
def gen_train_and_test_features(rules, names, X_train, X_test):
    ...
    for i, ruleset in enumerate(rules):
        X_train_rules = gen_rules(X_train, ruleset)
        X_test_rules = gen_rules(X_test, ruleset)
